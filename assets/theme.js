...
(function ($) {
  ...
  theme.ProductScreen = (function () {
    ...
    ProductScreen.prototype.init = function () {
      ...
      this.$screen
        ...
        .on("click", { noAnimate: true, back: true }, this.close.bind(this));
    ...
    Drawer.prototype = $.extend({}, Drawer.prototype, {
      ...
      open: function (evt, returnFocusEl) {
        ...
        if (evt && evt.stopPropagation) {
          evt.stopPropagation();
          // save the source of the click, we'll focus to this on close
          this.$activeSource = $(evt.currentTarget).attr(
            "aria-expanded",
            "true"
          );
  ...
  theme.AjaxCart = (function () {
    ...
    AjaxCart.prototype = $.extend({}, AjaxCart.prototype, {
      init: function () {
        ...
        this.$propertyTemplate = $(selectors.itemProperty, this.$form)
          .first()
          .clone();
        this.$discountTemplate = $(selectors.itemDiscount, this.$form)
          .first()
          .clone();
        this.$cartDiscountTemplate = $(selectors.cartDiscount, this.$container)
          .first()
          .clone();
      },
      ...
      initEventListeners: function () {
        ...
        this.$container.on(
          "click",
          selectors.itemDelete,
          this._onItemDelete.bind(this)
        );
        this.$container.on(
          "input",
          selectors.itemInputQuantity,
          $.debounce(500, this._onItemQuantityChange.bind(this))
        );
        this.$container.on(
          "blur",
          selectors.itemInputQuantity,
      ...
      _createCart: function (cart) {
        ...
        $(selectors.item, $form).not(selectors.cartNoteContainer).remove();

        $(selectors.itemList, $form).prepend(this._createItemList(cart));

        $(selectors.cartNoteInput, $form).val(cart.note);

        $(selectors.cartDiscountContainer, $form).toggleClass(
          "hide",
        ...
        $(selectors.cartSubtotal, $form).html(
        ...
        $(selectors.cartSubmit, $form).attr(
          "disabled",
          cart.items.length === 0
        );
        ...
        return $.map(
          ...
          function (item) {
            var $item = this.$itemTemplate
              .clone()
              .removeClass(classes.cartTemplate);
            ...
            $item
              .find(selectors.itemId)
              .addBack(selectors.itemId)
              .attr(data.itemId, item.key);

            $(selectors.itemHref, $item).attr("href", item.url);
            ...
            $(selectors.itemBackgroundImage, $item)
              ...
              .css(
                ...
                item.image
                  ? "url(" +
                      theme.Images.getSizedImageUrl(item.image, "200x") +
                      ")"
                  : "none"
              )
              .addClass("lazyload");
            ...
            $(selectors.itemLinePrice, $item).html(
              theme.Currency.formatMoney(
                item.line_price,
                theme.settings.moneyFormat
              )
            ...
            $(selectors.itemLabelQuantity, $item).attr(
              "for",
              "quantity_" + item.key
            );
            ...
            $(selectors.itemPropertyList, $item).html(propertyList);

            $(selectors.itemDiscountList, $item).html(discountList);

            $(selectors.itemUnitPrice, $item).html(unitPrice);
      ...
      _onItemDelete: function (evt) {
        ...
        if ($items.length === 2 && $items.last().is($note)) {
          ...
          theme.a11y
            .promiseTransitionEnd($(selectors.itemList, this.$container))
            .then(function () {
        ...
        theme.a11y.promiseAnimationEnd($item).then(
          function () {
  ...
  theme.StickyCart = (function () {
    ...
    function refresh(cart) {
      ...
      $(selectors.items).text(
        theme.strings.cartItems.replace("[count]", cart.item_count)
      );
      $(selectors.subtotal).html(
        theme.Currency.formatMoney(cart.total_price, theme.settings.moneyFormat)
      );
    }
    ...
    StickyCart.prototype = $.extend({}, StickyCart.prototype, {
      ...
      showCart: function (count, subtotal) {
        if (count) {
          $(selectors.items).text(
            theme.strings.cartItems.replace("[count]", count)
          );
        }
        if (subtotal) {
          $(selectors.subtotal).html(
      ...
      $("body")
        ...
        .on("click" + namespace, selectors.trigger, function () {
          ...
          if (isTab) {
            ...
            $(selectors.tabs + "[data-id=" + $el.data("id") + "]").each(
              function () {
  ...
  theme.headerNav = (function () {
    ...
    function init() {
      ...
      // Reset config
      config.overlayHeader = theme.settings.overlayHeader = $siteHeader.data(
        "overlay"
      );
      ...
      $window.on(
        ...
        $.throttle(150, stickyMenuOnScroll)
      );
    ...
    function setHeaderStyle() {
      // Remove to always have sticky header on mobile too
      //
    ...
    function toggleBarMenu(active) {
      ...
      if (active) {
        ...
        $siteHeader
          .removeClass(classes.openTransitionClass)
          .removeClass(classes.stickyClass);
  ...
  theme.slideNav = (function () {
    ...
    function toggleSubNav(evt) {
      ...
      if (isReturn) {
        ...
        $('.slide-nav__dropdown[data-level="' + menuLevel + '"]')
          .prepareTransition()
          .removeClass(classes.isActive);
  ...
  theme.QuickShopScreens = (function () {
        ...
        function (evt) {
          ...
          if (location.pathname.indexOf("/products/") !== -1) {
            if (currentScreen) {
              ...
              $('.quick-product__btn[href="' + location.pathname + '"]')
                .first()
                .trigger("click", { updateCurrentPath: false });
            }
    ...
    function getProductIds($scope) {
      ...
      $triggers.each(function () {
        ...
        if (ids.indexOf(id) > -1) {
          $('.screen-layer--product[data-product-id="' + id + '"]')
            .slice(1)
            .remove();
          return;
        }
    ...
    function unload($container) {
      ...
      var $activeButtons = $(selectors.trigger).not(
        $(selectors.trigger, $container)
      );
    ...
    function removeDuplicateModals(ids, $container) {
      for (var i = 0; i < ids.length; i++) {
        $(
          '.screen-layer--product[data-product-id="' + ids[i] + '"]',
          $container
        ).remove();
      }
    }
  ...
  theme.HoverProductGrid = (function () {
    ...
    HoverProductGrid.prototype = $.extend({}, HoverProductGrid.prototype, {
        ...
        if (theme.config.bpSmall) {
          if (this.slidersMobile) {
            $(window).on(
              ...
              $.throttle(120, this.inViewSliderInit.bind(this))
            );
      ...
      inViewSliderInit: function () {
        this.$products.find(selectors.slider).each(
          function (i, el) {
      ...
      initSlider: function ($slider) {
        ...
        $slider.addClass("product-slider--init").slick({
          autoplay: true,
          infinite: true,
          arrows: false,
          speed: 300,
  ...
  theme.Product = (function () {
    ...
    Product.prototype = $.extend({}, Product.prototype, {
      ...
      availabilitySetup: function () {
        var availabilityContainer = this.$container[0].querySelector(
      ...
      initVariants: function () {
        ...
        if ($(selectors.variantColorSwatch, this.$container).length) {
          $(selectors.variantColorSwatch, this.$container).on(
            "change",
            function (evt) {
              var $el = $(evt.currentTarget);
        ...
        if (
          ...
          $currentVariantJson.length
        ) {
          this.currentVariantObject = JSON.parse(
            $currentVariantJson[0].innerHTML
          );
      ...
      initAjaxProductForm: function () {
        if (
          ...
          theme.settings.cartType === "sticky"
        ) {
        ...
        if (variant) {
          if (variant.available) {
            ...
            $(selectors.addToCart, this.$container)
              ...
              .prop("disabled", false);
            var defaultText = $(selectors.addToCartText, this.$container).data(
              "default-text"
            );
      ...
      updatePrice: function (evt) {
        ...
        if (variant) {
          // Regular price
          $(selectors.price, this.$container)
            .html(
              theme.Currency.formatMoney(
                variant.price,
                theme.settings.moneyFormat
          ...
          if (variant.compare_at_price > variant.price) {
            ...
            $(selectors.comparePriceA11y, this.$container).attr(
              "aria-hidden",
              "false"
            );
            $(selectors.priceA11y, this.$container).attr(
              "aria-hidden",
              "false"
            );
          } else {
      ...
      updateUnitPrice: function (evt) {
        ...
        if (variant && variant.unit_price) {
          ...
          $(selectors.unitWrapper, this.$container).addClass(
            classes.visuallyHide
          );
        }
      },
      ...
      setCurrentVariantAvailability: function (variant) {
        ...
        this.disableVariantGroup(
          $(selectors.formContainer, this.$container).find(
            ".variant-input-wrap"
          )
        );
      ...
      updateVariantAvailability: function (evt, value, index) {
        ...
        $(selectors.formContainer, this.$container)
      ...
      enableVariantOptionByValue: function (array, index) {
        var $group = $(selectors.formContainer, this.$container).find(
      ...
      imageSetup: function (needStylesheet) {
        if (
          ...
          $(selectors.photoThumbs, this.$container).length < 2
        ) {
          // Single product image. Init video if it exists
          var $video = $(selectors.productImageMain, this.$container).find(
      ...
      updateVariantImage: function (evt) {
        ...
        var imageIndex = $(
          '.product__slide[data-id="' + variant.featured_media.id + '"]'
        ).data("slick-index");
        ...
        $(selectors.photoThumbs, this.$container)
          ...
          .on(
            ...
            function (evt) {
              ...
              if (!theme.config.bpSmall) {
                if (this.settings.stackedImages) {
                  $(selectors.photoThumbItem, this.$container).removeClass(
                    classes.thumbActive
                  );
                  this.stackedScrollTo(index);
                } else {
      ...
      stackedImagesInit: function () {
        ...
        $(window).on(
          ...
          $.debounce(200, this.stackedImagePositions.bind(this))
        );
        ...
        $(window).on(
          ...
          $.throttle(
            ...
            function () {
              ...
              var closest = this.settings.stackedImagePositions.reduce(
                function (prev, curr) {
      ...
      stackedActive: function (index) {
        $(selectors.photoThumbItem, this.$container)
          ...
          .eq(index)
          .addClass(classes.thumbActive);

        if (this.settings.hasVideos) {
          this.stopVideo();

          var $video = $(selectors.photo, this.$container)
            .eq(index)
            .find(".product__video");
        ...
        if (this.settings.has3d) {
          ...
          var $media = $(selectors.photo, this.$container)
            .eq(index)
            .find(selectors.media);
        ...
        if (!this.settings.modalInit) {
          ...
          $.get(
            ...
            function (data) {
              ...
              var $sectionDiv = $template.find(
                "#ProductSections-" + this.sectionId
              );
              if ($sectionDiv.length) {
                this.loadProductSections($sectionDiv);
              }

              var $relatedDiv = $template.find(
      ...
      initModelViewerLibraries: function () {
        var $modelViewerElements = $(selectors.media, this.$container);
      ...
      customMediaListners: function () {
        $(selectors.closeMedia, this.$container).on(
          "click",
          function () {
            this.$container.find(selectors.media).trigger("mediaHidden");
          }.bind(this)
        ...
        updateContent: function (variantId) {
          ...
          fetch(variantSectionUrl)
            ...
            .then(function (storeAvailabilityHTML) {
              ...
              var drawerId = self.container
                .querySelector(selectors.drawerOpenBtn)
                .getAttribute("aria-controls");

              self.drawer = new theme.Drawers(drawerId, "availability");

              drawerHolder.querySelector(selectors.productTitle).textContent =
                self.productTitle;
            });
        },
      }
  ...
  theme.FeaturedCollection = (function () {
    ...
    function FeaturedCollection(container) {
      ...
      this.settings = {
        ...
        page: 1,
        pageCount: this.options.paginate
          ? Math.ceil(productCount / paginateBy)
          : 0,
        itemsToScroll: 3,
      ...
      theme.utils.promiseStylesheet().then(
        function () {
          ...
          $(window).on(
            "scroll" + this.namespace,
            $.debounce(200, this.checkVisibility.bind(this))
          );
        }.bind(this)
      );
    }
    ...
    FeaturedCollection.prototype = $.extend({}, FeaturedCollection.prototype, {
      ...
      init: function () {
        ...
        $(window).on(
          ...
          $.debounce(200, this.handleResize.bind(this))
        );
      ...
      toggleScrollListener: function (enable) {
        if (enable) {
          ...
          this.$scrollWrap.on(
            "scroll" + this.namespace,
            $.throttle(250, this.scrollCheck.bind(this))
          );
      ...
      scrollCheck: function (evt) {
        ...
        var scrollLeft = evt.currentTarget.scrollLeft
      ...
      arrowScroll: function (evt) {
        var direction = $(evt.currentTarget).hasClass(classes.arrowLeft)
          ? "left"
          : "right";
        ...
        if (evt.type === "mouseenter") {
          ...
          var currentPos = this.$scrollWrap.scrollLeft();
          var scrollAmount =
            this.sizing.itemWidth * this.settings.itemsToScroll;
          var newPos =
            direction === "left"
      ...
      getNewProducts: function () {
        ...
        $.get(
          ...
          function (data) {
            var $template = $(data);
            var $newProducts = $template.find(
              selectors.collectionProductContainer + " .grid-product"
            );
            ...
            if (this.settings.combineTags) {
              if ($parent.hasClass(classes.activeTag)) {
                ...
                if ($parent.hasClass(classes.removeTagParent)) {
                  ...
                  $(selectors.activeTagList).append(
    ...
    function HeroAnimated(container) {
      ...
      theme.utils.promiseStylesheet().then(
        function () {
          ...
          $(window).on(
            "scroll" + namespace,
            $.throttle(300, this.checkVisibility.bind(this))
          );
        }.bind(this)
      );
    }
